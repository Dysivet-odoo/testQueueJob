...
class SyncJob(models.Model):
    ...
    def _compute_state(self):
        ...
        for r in self:
            ...
            print('computed_state =',computed_state)
    ...
    def _compute_trigger_name(self):
        # delete_my_code
        print('- sync_job _compute_trigger_name')
        for r in self:
            print('r =',r)
            print('r.parent_job_id =',r.parent_job_id)
            if r.parent_job_id:
                r.trigger_name = (r.parent_job_id.trigger_name or "") + "." + r.function
                print('r.trigger_name =',r.trigger_name)
                continue
            for f in TRIGGER_FIELDS:
                t = getattr(r, f)
                if t:
